...
def test_nested_calls():
    ...
    assert len(call_sites) >= 3, f"Expected at least 3 call sites, got {len(call_sites)}"
    ...
    print(f"✓ Found {len(call_sites)} distinct call sites")
...
def test_recursive_function():
    ...
    # Find the factorial call site (line 56 where report.log is called)
    factorial_site = None
    for site in call_sites:
        filename, lineno = site
        if lineno == 56:  # Line where report.log is called inside factorial
            factorial_site = site
            break
    
    assert factorial_site is not None, f"Could not find factorial call site. Found sites: {call_sites}"
    
    # Should have 4 log entries (one for each call: 4, 3, 2, 1)
    pickled_values = logs[factorial_site]
    assert len(pickled_values) >= 4, f"Expected at least 4 values, got {len(pickled_values)}"
    
    print("✓ Recursive function test passed")
